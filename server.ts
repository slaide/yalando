    ...
    shoe_keys=shoe_ids.slice().filter(key=>{
      ...
      if (!shoe_fits_gender(shoe,user.gender.name)){
        continue;
  		...
  		if(user_query_words.indexOf("soccer")==-1 && user_query_words.indexOf("football")==-1 && shoe.category=="FOOTBALL/SOCCER") discard_shoe=true;
  		if(user_query_words.indexOf("basketball")==-1 && shoe.category=="BASKETBALL") discard_shoe=true;
  		if(user_query_words.indexOf("football")==-1 && shoe.category=="AMERICAN FOOTBALL") discard_shoe=true;
