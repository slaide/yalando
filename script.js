var app=new Vue({
    ...
    data:{
		...
		preferences:[
			{name:"Review",description:"These shoes are described similarly by other users.",shoe_previews:[],show_shoe:false,current_preview:{},real:true},
			{name:"Interaction",description:"These shoes are often on the same wishlist.",shoe_previews:[],show_shoe:false,current_preview:{},real:true},
			{name:"Image",description:"These shoes look similar.",shoe_previews:[],show_shoe:false,current_preview:{},real:true},
			{name:"",description:"",shoe_previews:{},show_shoe:false,current_preview:{},real:false},
			{name:"",description:"",shoe_previews:{},show_shoe:false,current_preview:{},real:false},
			{name:"",description:"",shoe_previews:{},show_shoe:false,current_preview:{},real:false},
		],
		current_preference:{},
		current_shoe:{},
		recommend:true,
		blank_shoe:{id:"",name:"",price:"",price_text:"",bullets:[],src:"",click_text:{},real:false},
    },
    methods:{
						...
						if(sender.readyState==XMLHttpRequest.DONE){
							if(sender.status>=200 && sender.status<400){
							    try{
    								...
    								if(app.preferences[0].name!="Review") throw "pref 0 is not review";
    								app.preferences[0].shoe_previews=new_shoes.review;
    								if(app.preferences[1].name!="Interaction") throw "pref 1 is not interaction";
    								app.preferences[1].shoe_previews=new_shoes.interaction;
    								if(app.preferences[2].name!="Image") throw "pref 2 is not image";
    								app.preferences[2].shoe_previews=new_shoes.image;
									
									//app.peekPreference(app.preferences[0]);
								}catch(e){
									console.log("invalid server data pref",e);
								}
							}
						}
					};
						...
						if(sender.readyState==XMLHttpRequest.DONE){
							if(sender.status>=200 && sender.status<400){
							    try{
									...
									console.log("invalid server data shoes",e);
								}
							}
						}
					};
